...
import { Calculator, Database, Handshake, Link2, Repeat2, Sparkles, Wrench, X } from "lucide-react";
...
export function ProgressDashboard({ indicators }: { indicators: Indicator[] }) {
  ...
  const capacityIndicators = [
    { id: "indicator-resident-actions", icon: Sparkles, label: "居民发起" },
    { id: "indicator-resource-match", icon: Handshake, label: "资源连接" },
    { id: "indicator-repeat-participation", icon: Repeat2, label: "持续参与" },
    { id: "indicator-maintenance", icon: Wrench, label: "长期维护" },
  ].map((item) => ({ ...item, indicator: indicators.find((indicator) => indicator.id === item.id) })).filter((item) => item.indicator);
  ...
  return (
    <>
      <section className="community-capacity-panel"><div className="card-heading"><div><h2>社区行动能力</h2><p>不只看办结了多少事项，也看居民是否能够组织行动、找到资源并持续维护。</p></div><DemoDataBadge /></div><div className="community-capacity-grid">{capacityIndicators.map(({ id, icon: Icon, label, indicator }) => indicator ? <button key={id} onClick={() => setSelected(indicator)}><Icon size={21} /><span>{label}</span><strong>{indicator.value}<small>{indicator.unit}</small></strong><p>{indicator.name}</p><i>{indicator.change}</i></button> : null)}</div></section>
