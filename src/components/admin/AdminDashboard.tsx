...
import { Activity, BarChart3, CheckCircle2, ChevronRight, ClipboardCheck, Download, FileCheck2, FolderKanban, Gauge, LayoutDashboard, MessageSquare, ScrollText, ShieldCheck, UsersRound, XCircle } from "lucide-react";
import { useMemo, useState } from "react";
import { useDemo } from "@/components/providers/DemoProvider";
import { DemoDataBadge } from "@/components/common/DemoDataBadge";
import { StatusBadge } from "@/components/common/StatusBadge";
import { can } from "@/lib/permissions";
import { issueStatusLabels, reviewStatusLabels } from "@/lib/utils";
import { contentService } from "@/services/content";
import { activityService } from "@/services/activities";
import { indicatorService } from "@/services/indicators";
import { projectService } from "@/services/projects";
import { IssueStatus, ReviewStatus, UserRole } from "@/types";
...
export function AdminDashboard() {
  const { role, setRole, issues, updateIssueStatus, notify } = useDemo();
  ...
  const stats = useMemo(() => ({ pending: issues.filter((item) => item.status === IssueStatus.Pending).length, processing: issues.filter((item) => [IssueStatus.Accepted, IssueStatus.Assigned, IssueStatus.Processing].includes(item.status)).length, research: research.filter((item) => (researchStatuses[item.id] ?? item.status) === ReviewStatus.Pending).length }), [issues, research, researchStatuses]);
  ...
  const exportData = () => {
    const blob = new Blob([JSON.stringify({ exportedAt: new Date().toISOString(), demo: true, issues, projects, indicators }, null, 2)], { type: "application/json" });
  ...
  return (
    <div className="admin-shell">
      <aside className="admin-sidebar"><div className="admin-brand"><span className="brand-seal">红</span><div><strong>平台管理后台</strong><small>DEMO CONSOLE</small></div></div><nav>{sections.map((item) => { const Icon = item.icon; return <button key={item.id} className={section === item.id ? "active" : ""} onClick={() => setSection(item.id)}><Icon size={18} />{item.label}{item.id === "issues" && stats.pending ? <span>{stats.pending}</span> : null}</button>; })}</nav><button className="button button-secondary export-button" onClick={exportData}><Download size={17} />导出演示数据</button></aside>
