...
import { isHongtangVectorProfile } from "../../lib/ui-profile";
...
export function DesktopShell({
  ...
  useEffect(() => {
    ...
    enforceViewerPlugins();
    // The focused Hongtang workspace is an editor, not a generic GIS viewer.
    // Keep the point/line/polygon toolbar ready without asking non-technical
    // users to discover and enable the GeoEditor plugin first.
    if (isHongtangVectorProfile() && !pluginManager.isActive(GEO_EDITOR_PLUGIN_ID)) {
      void Promise.resolve(pluginManager.activate(GEO_EDITOR_PLUGIN_ID, appAPI)).catch((error) => {
        const normalized = error instanceof Error ? error : new Error(String(error));
        appendDiagnostic({
          category: "runtime",
          level: "error",
          message: `Could not activate the focused vector editor: ${normalized.message}`,
          detail: normalized.stack,
          source: `plugin:${GEO_EDITOR_PLUGIN_ID}`,
        });
      });
    }
