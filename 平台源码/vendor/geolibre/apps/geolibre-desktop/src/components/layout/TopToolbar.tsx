...
export function TopToolbar({
  ...
  const shortcutCommands = useMemo(() => {
    if (viewer) return commands.filter((command) => command.id.startsWith("view."));
    if (!hongtangVectorProfile) return commands;
    return commands.filter(
      (command) =>
        command.id.startsWith("project.") ||
        command.id.startsWith("edit.") ||
        command.id.startsWith("view."),
    );
  }, [commands, hongtangVectorProfile, viewer]);
  useGlobalShortcuts({
    commands: shortcutCommands,
    onOpenPalette: viewer || hongtangVectorProfile ? undefined : () => setCommandPaletteOpen(true),
    onOpenShortcuts: viewer || hongtangVectorProfile ? undefined : () => setShortcutsOpen(true),
  });
  ...
  const appTitle = hongtangVectorProfile
    ? "红塘矢量编辑"
    : isTauri() && !isMobile()
      ? "GeoLibre Desktop"
      : "GeoLibre";
  ...
  return (
    ...
    >
      ...
      {/* No plugin marketplace in the Mac App Store build (all its entry
          ...
          bundle). */}
      {!IS_MAS_BUILD && !hongtangVectorProfile && (
        <ManagePluginsDialog
