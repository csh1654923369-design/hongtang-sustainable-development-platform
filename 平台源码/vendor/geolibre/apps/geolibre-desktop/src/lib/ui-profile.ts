...
const HONGTANG_VECTOR_PROFILE = "hongtang-vector";

const HONGTANG_VECTOR_DATA_SOURCES = new Set([
  "vector",
  "delimited-text",
  "gpx",
  "wfs",
...
const HONGTANG_VECTOR_MENUS = new Set(["project", "edit", "view", "addData"]);

const HONGTANG_VECTOR_MENU_ITEMS = new Set([
  "project.new",
  "project.openFrom",
  "project.openRecent",
  "project.history",
  ...
  "view.resetPitch",
  "view.resetNorth",
  "view.resetPitchBearing",
  "view.setView",
]);

/**
 * 红塘平台使用的专用矢量编辑界面。它由 URL 显式开启，不受浏览器中
 * 既有的 GeoLibre 个人设置影响，确保不同电脑看到一致、精简的工具集。
 */
export function isHongtangVectorProfile(): boolean {
  if (typeof window === "undefined") return false;
  return new URLSearchParams(window.location.search).get("profile") === HONGTANG_VECTOR_PROFILE;
}
...
export function isDataSourceVisible(profile: UiProfileSettings, id: string): boolean {
  if (isHongtangVectorProfile()) return HONGTANG_VECTOR_DATA_SOURCES.has(id);
...
export function isPluginVisible(profile: UiProfileSettings, id: string): boolean {
  if (isHongtangVectorProfile()) return id === "maplibre-gl-geo-editor";
...
export function isMenuVisible(profile: UiProfileSettings, menuId: string): boolean {
  if (isHongtangVectorProfile()) return HONGTANG_VECTOR_MENUS.has(menuId);
...
export function isMenuItemVisible(profile: UiProfileSettings, itemId: string): boolean {
  if (isHongtangVectorProfile()) return HONGTANG_VECTOR_MENU_ITEMS.has(itemId);
