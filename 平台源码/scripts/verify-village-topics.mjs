...
try {
  ...
  const closedTopicMotion = await topicCard.evaluate((element) => {
    ...
    return {
      ...
      opacity: panel ? getComputedStyle(panel).opacity : "missing",
      transitionProperty: style.transitionProperty,
    };
  });
  assert.equal(closedTopicMotion.gridRows.split(" ").at(-1), "0px");
  assert.equal(closedTopicMotion.opacity, "0");
  assert.match(closedTopicMotion.transitionProperty, /grid-template-rows/);
  ...
  assert.notEqual(openTopicMotion.gridRows.split(" ").at(-1), "0px");
