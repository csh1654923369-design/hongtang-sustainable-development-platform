...
if (-not (Test-Path -LiteralPath $localEnvFile)) {
  ...
  $sharedSourceRoot = Join-Path $workspaceRoot "平台源码"
  Import-EnvironmentFile (Join-Path $sharedSourceRoot ".env.local")
  Import-EnvironmentFile (Join-Path $sharedSourceRoot ".env.development.local")
}
