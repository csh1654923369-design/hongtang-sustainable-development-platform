...
export interface AmapMapInstance {
  ...
  containerToLngLat(pixel: AmapPixel): { getLng(): number; getLat(): number };
...
function loadAmapClientConfig(): Promise<AmapClientConfig> {
  const key = process.env.NEXT_PUBLIC_AMAP_WEB_KEY?.trim();
  if (key) {
  ...
  return import("@/lib/sitePath").then(({ sitePath }) =>
    fetch(sitePath("/api/amap-config"), { cache: "no-store" }).then(async (response) => {
...
export function loadAmap() {
  ...
  amapLoadPromise = loadAmapClientConfig()
    .then((config) => {
...
/** Convert the GCJ-02 coordinates returned by AMap back to source WGS84. */
export function gcj02ToWgs84(longitude: number, latitude: number): AmapCoordinate {
  if (outsideChina(longitude, latitude)) return [longitude, latitude];
  const [shiftedLongitude, shiftedLatitude] = wgs84ToGcj02(longitude, latitude);
  return [longitude * 2 - shiftedLongitude, latitude * 2 - shiftedLatitude];
}
