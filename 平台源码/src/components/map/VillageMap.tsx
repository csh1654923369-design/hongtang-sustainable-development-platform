...
export const geographicBasemaps: Record<VillageBasemap, { bounds: GeographicBounds; src: string; alt: string; width: number; height: number; title: string; note: string }> = {
  handdrawn: {
    ...
    src: sitePath("/data/hongtang-handdrawn-map.png"),
    alt: "红塘村手绘地图",
    width: 2048,
    height: 2870,
    title: "红塘村手绘地图",
  ...
  aerial: {
    ...
    src: sitePath("/data/hongtang-orthophoto-0.3m.webp"),
    alt: "红塘村无人机正射影像",
    width: 3357,
    height: 3554,
    title: "红塘村无人机正射影像",
...
export function VillageMap({
  ...
  const selectedPosition = useMemo(() => {
    ...
    if (selectedTopicSpatialId && topicSpatial) {
      ...
      if (selection) {
        const [longitude, latitude] = topicFeatureCenter(selection.item);
    ...
    return undefined;
  }, [features, geographic.bounds, selectedId, selectedSpatialId, selectedTopicSpatialId, topicSpatial, waterSystem]);

  useEffect(() => {
    if (selectedPosition) focusAt(selectedPosition.x, selectedPosition.y);
  }, [focusAt, selectedId, selectedPosition, selectedSpatialId, selectedTopicSpatialId]);
    ...
    <div className={`village-map basemap-${basemap} ${interactiveLocation ? "location-mode" : ""}`} onClick={clickMap}>
      ...
      >
        ...
        >
          ...
          />
          {topicSpatial ? (
            <svg className="map-geographic-vector topic-spatial-vector" viewBox={`0 0 ${geographic.width} ${geographic.height}`} preserveAspectRatio="none" aria-label="topic spatial vector layers">
              {topicSpatial.features.map((item) => {
                ...
                const points = item.geometry.coordinates.map(([longitude, latitude]) => project(longitude, latitude).join(",")).join(" ");
                return item.geometry.type === "Polygon"
                  ? <polygon key={item.id} className={selectedTopicSpatialId === item.id ? "active" : ""} points={points} fill={layer.color} stroke={layer.color} role="button" tabIndex={0} onClick={() => onSelectTopicSpatial?.(selection)} onKeyDown={(event) => { if (event.key === "Enter" || event.key === " ") onSelectTopicSpatial?.(selection); }}><title>{item.title}</title></polygon>
                  : <g key={item.id} role="button" tabIndex={0} onClick={() => onSelectTopicSpatial?.(selection)} onKeyDown={(event) => { if (event.key === "Enter" || event.key === " ") onSelectTopicSpatial?.(selection); }}><polyline className="home-water-line-hit" points={points} /><polyline className={selectedTopicSpatialId === item.id ? "active" : ""} points={points} stroke={layer.color}><title>{item.title}</title></polyline></g>;
              })}
            </svg>
          ) : null}
          {waterSystem ? (
