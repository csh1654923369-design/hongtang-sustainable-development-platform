...
OUTPUT = QA_ROOT / "红塘村可持续发展平台使用手册.v122.docx"
BACKUP = QA_ROOT / "红塘村可持续发展平台使用手册.V1.21.backup.docx"
...
if "V1.21 Demo" not in document.paragraphs[5].text:
    raise ValueError(f"Expected V1.21 manual, got: {document.paragraphs[5].text}")
if not BACKUP.exists():
    shutil.copy2(MANUAL, BACKUP)

document.paragraphs[5].text = (
    "版本：V1.22 Demo   |   更新日期：2026年7月28日   |   适用地址：http://localhost:3000"
)

operation = find_paragraph_prefix(document, "3D首页操作：")
operation.text = (
    "3D首页操作：打开首页后，平台会依次加载周边三维地形、远景卫星影像、红塘无人机正射影像和高斯模型。"
    "高斯模型外侧显示连续山地环境，不再显示黑色背景。平台会等待高斯模型表面加载完成后再放置5个事项图钉；"
    "图钉优先贴合高斯实景表面，同时检查当地三维地形高度，若拾取位置低于地形则自动抬升到地表之上，"
    "避免模型边缘的图钉钻入地下或跳到错误高度。按住左键拖动，可沿村庄所在地表的水平面平移。"
...
technical.text = (
    ...
    "Cesium World Imagery；红塘0.3米无人机正射影像按真实WGS84范围铺到地形表面。"
    "高斯资产根变换的原点为99.908740607°E、24.636255278°N、1764m，模型、地形和影像使用同一地理坐标空间。"
    "三维图钉不再在查看器初始化时立即计算，而是在initialTilesLoaded事件之后沿局部竖直方向调用"
    "pickFromRayMostDetailed拾取高斯表面；随后调用sampleTerrainMostDetailed取得同一经纬度的地形高度。"
    "最终锚点高度取高斯表面与地形表面的较高值，再增加2.5米至6米的图钉抬升量。"
    "这样即使模型边缘未命中、瓦片加载顺序变化或地形局部高于高斯表面，图钉也不会进入地下。"
    "自动测试会校验5个图钉均在初始画面内、优先来自高斯表面且最终高度高于当地地形。"
)
...
for cell, value in zip(
    ...
    [
        "V1.22 Demo",
        "2026-07-28",
        "修复小花园等三维图钉偶发偏位或钻地：等待高斯表面加载后再定位，并以当地地形高度作为最低高度保护。",
    ],
):
    cell.text = value
...
assert [len(table.rows) for table in check.tables] == [5, 22, 9, 13, 24]
assert "V1.22 Demo" in check.paragraphs[5].text
all_text = "\n".join(paragraph.text for paragraph in check.paragraphs)
assert "若拾取位置低于地形则自动抬升到地表之上" in all_text
assert "pickFromRayMostDetailed" in all_text
assert "sampleTerrainMostDetailed" in all_text
assert any(row.cells[0].text.strip() == "V1.22 Demo" for row in check.tables[4].rows)
...
print(f"Updated manual to V1.22: {MANUAL}")
